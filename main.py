...
def create_board(n):
    ...
    print(f"Initial chess board {N_QUEENS}x{N_QUEENS}:")
    print('\n'.join([". "*value + "Q " + ". "*(N_QUEENS-value-1) for value in chess_board.values()]))
    return chess_board
...
def print_chess_board(board):
    ...
    print(f"\nChess board {N_QUEENS}x{N_QUEENS} output:")
    print('\n'.join([". "*value + "Q " + ". "*(N_QUEENS-value-1) for value in board.values()]))
    print("\nValues to check correct output:")
    print('\n'.join(["{} => {}".format(column, row) for column, row in board.items()]))


def main():
    start = time.time()
    simulated_annealing()
    print(f"\nRuntime in second: {round(time.time() - start, 3)} seconds")
